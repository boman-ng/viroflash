...
Pass 1 validates the complete FASTQ input, checks PE counts and normalized IDs, computes exact fragment count `N`, and establishes the compressed-input digest from the same byte stream. Pass 2 repeats validation and digesting on the analysis stream, rejects any cross-pass byte change, and deterministically selects fragments with a domain-separated BLAKE3 key based on profile identity, input identity, fragment ID, and ordinal. Configured alignment workers consume a bounded queue and results are merged by fragment ordinal.
